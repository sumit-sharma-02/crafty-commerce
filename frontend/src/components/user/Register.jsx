  ...
  const submitHandler = (event) => {
    ...
    if (!isCorrectEmail(user.email)) {
      return showErrorToast(
        "Incorrect email format. The email must contain '.' and '@' special character, Other special characters are not allowed."
      );
    } else if (!isCorrectPassword(user.password)) {
      return showErrorToast(
        "Incorrect password formatt. The password must contain 6 characters with at least 1 lowercase, 1 uppercase, 1 numeric and 1 special character"
      );
